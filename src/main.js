...
function createWindow() {
  window = new BrowserWindow({
    ...
    title: 'Proxyking', backgroundColor: '#0c1018', autoHideMenuBar: true,
...
if (primary) app.whenReady().then(async () => {
  ...
  handle('capture:certificate', async () => {
    if (!engine.certificatePath) throw new Error('Start capture once to create your local certificate.');
  ...
  handle('capture:export', async () => {
    const result = await dialog.showSaveDialog(window, { title: 'Export captured session', defaultPath: `Proxyking-${new Date().toISOString().replace(/[:.]/g, '-')}.har`, filters: [{ name: 'HTTP Archive', extensions: ['har'] }] });
    if (result.canceled) return false;
    await fs.writeFile(result.filePath, JSON.stringify(engine.exportHar(), null, 2));
    return true;
  });
  createWindow();
