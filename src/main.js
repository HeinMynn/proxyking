...
const { installMacCertificate, removeMacCertificate, macCertificateTrustStatus } = require('./certificate-trust');
...
if (primary) app.whenReady().then(async () => {
  ...
  handle('capture:trust-certificate', async () => {
    if (process.platform !== 'darwin') throw new Error('Install & Trust CA is currently available only on macOS.');
    const certificatePath = await engine.prepareCertificate();
    await installMacCertificate(certificatePath);
    return true;
  });
  handle('capture:certificate-status', async () => {
    if (process.platform !== 'darwin') return { state: 'unsupported', label: 'Manual setup' };
    return macCertificateTrustStatus(await engine.prepareCertificate());
  });
  handle('capture:remove-certificate', async () => {
    if (process.platform !== 'darwin') throw new Error('Revoke & Remove CA is currently available only on macOS.');
    const confirmation = await dialog.showMessageBox(window, {
      type: 'warning', title: 'Revoke Proxyking CA?',
      message: 'Remove Proxyking Local CA from your login keychain?',
    ...
    return removeMacCertificate(await engine.prepareCertificate());
  });
