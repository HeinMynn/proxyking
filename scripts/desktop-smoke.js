...
(async () => {
  ...
  try {
    ...
    await expect(page.locator('#settingsConnection')).toBeVisible();
    await page.locator('[data-settings-section="exclusions"]').click();
    await expect(page.locator('#settingsExclusions')).toBeVisible();
    ...
    await page.locator('#emptySetup').click();
    await expect(page.locator('#settingsConnection')).toBeVisible();
    ...
    await page.locator('.request-row').click({ button: 'right' });
    await expect(page.locator('#connectionContextMenu')).toBeVisible();
    await expect(page.locator('#connectionContextMenu [data-action="exclude-host"]')).toBeVisible();
    await expect(page.locator('#connectionContextMenu [data-action="exclude-app"]')).toContainText('Google Chrome');
    await page.keyboard.press('Escape');
    await expect(page.locator('#connectionContextMenu')).toBeHidden();
    await assertFooterAtBottom();
    ...
    await page.locator('[data-side="request"] [data-view="raw"]').click();
    await expect(page.locator('#requestRawSearch')).toBeVisible();
    await page.locator('#requestRawSearch').fill('source=desktop');
    await expect(page.locator('#requestContent mark.filter-text-match')).toContainText('source=desktop');
    await page.locator('[data-side="response"] [data-view="raw"]').click();
    await expect(page.locator('#responseRawSearch')).toBeVisible();
    await page.locator('#responseRawSearch').fill('Hello from Proxyking');
    ...
    await expect(page.locator('#responseRawSearch')).toBeHidden();
    ...
    await expect(page.locator('#advancedFilterToggle')).toHaveAttribute('aria-expanded', 'true');
    await page.keyboard.press('Escape');
    await expect(page.locator('#advancedFilterPanel')).toBeHidden();
    await expect(page.locator('#advancedFilterToggle')).toHaveAttribute('aria-expanded', 'false');
    await page.keyboard.press(process.platform === 'darwin' ? 'Meta+f' : 'Control+f');
    await expect(page.locator('#advancedFilterPanel')).toBeVisible();
    await expect(page.locator('#advancedFilterMode')).toHaveCount(0);
    const filterPanelBox = await page.locator('#advancedFilterPanel').boundingBox();
    const mainAreaBox = await page.locator('#mainArea').boundingBox();
    assert.ok(filterPanelBox && mainAreaBox && Math.abs(filterPanelBox.y - mainAreaBox.y) < 1 && Math.abs(filterPanelBox.height - mainAreaBox.height) < 1, `Advanced filter panel does not fill the main area height: ${JSON.stringify({ filterPanelBox, mainAreaBox })}`);
    ...
    await expect(page.locator('.request-row')).toHaveCount(1);
    await expect(page.locator('.request-row')).not.toHaveClass(/filter-match/);
    await expect(page.locator('#responseContent mark.filter-text-match')).toHaveCount(0);
    ...
    await page.locator('#settingsButton').click();
    await page.locator('[data-settings-section="https"]').click();
    await expect(page.locator('#settingsHttps')).toBeVisible();
    await expect(page.locator('#certificateStatusModal')).toBeVisible();
    ...
    await expect(page.locator('#windowsCliTrust')).toContainText('yt-dlp.exe --proxy ""');
