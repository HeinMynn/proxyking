...
1. Click **Start** in the header. Proxyking detects the private IPv4 address on the active network route (for example, `192.168.1.42`) and uses port `8080` by default. The port is configurable under **Settings → Connection**.
...
- Turn off **Configure system proxy automatically** under **Settings → Connection** to use manual mode. Then set your app's HTTP/HTTPS proxy to the exact address shown in the header, such as `192.168.1.42:8080`, and remove that configuration when finished. Apps with independent proxy settings, such as Firefox configurations, may need to select **Use system proxy settings**.
...
- The **Settings** workspace organizes connection and system-proxy controls, HTTPS certificate trust and command-line guidance, and persistent capture exclusions in separate sidebar sections. Exclusion rules accept exact hostnames, wildcard domains such as `*.telegram.org`, IP addresses, or detectable app names. Excluded traffic is forwarded but does not appear in the connection list, TUNNEL records, or HAR exports; excluded HTTPS hosts also skip inspection.
...
The Windows `yt-dlp.exe` build normally uses its bundled `certifi` CA list instead of the Windows certificate store. While Proxyking is active, run `yt-dlp.exe --compat-options no-certifi "URL"` so `yt-dlp` recognizes the trusted Proxyking CA without disabling TLS verification. Add `--compat-options no-certifi` to the yt-dlp configuration file if this should apply to every download.

To exclude `yt-dlp` from Proxyking entirely, run `yt-dlp.exe --proxy "" "URL"`. The explicit empty proxy overrides the active Windows system proxy for that invocation. This is more reliable than an app-name exclusion because an HTTPS client must accept Proxyking's TLS certificate before it can send identifying HTTP headers.
