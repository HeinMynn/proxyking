...
- The separate **Settings** page provides persistent **Exclude from capture** rules. Enter exact hostnames, wildcard domains such as `*.telegram.org`, IP addresses, or detectable app names. Excluded traffic is forwarded but does not appear in the connection list, TUNNEL records, or HAR exports; excluded HTTPS hosts also skip inspection. App names are learned from identifiable requests, after which later HTTPS connections to their detected hosts are excluded too.
